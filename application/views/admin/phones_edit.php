<div class="admin-title">Edit Phone 
...
<div class="admin-content">
	<div class="list-div">
		...
		if(issetNotEmpty($file_error)){
			...
			echo form_open_multipart('admin/phones/edit/'.$id, '', $hiddenlist ); 
			echo '<table width="100%" border="0" cellpadding="0" cellspacing="0" class="phone-add-table"> ';
			
			foreach($display_fields as $field => $label){	
				echo '<tr>';
					echo '<td class="valign-top">'. $label . '</td>';
					echo '<td>';
						if(isset($phone[$field])){ $val = ((set_value($field) != '') ? set_value($field):$phone[$field]); }
						/*
						...
						*/
						$option = array('name' => $field, 'value' => $val  );	
						...
						if($this->jm->isFK($field)){   
							...
							if($field == 'image'){
								$form_input = form_upload(array('name' => $field,'id'=> $field, 'value' => $val ));
								$form_input .= '<br/><br/><div class="italic">Image Dimension should be 125px(width) by 239px(height)</div>';
								$form_input .= '<br/><div>'. (issetNotEmpty($val) ? img('images/phones/'.$val) : img('images/phones/default-phone.png')).'</div>';
							} else if($field == 'descp') {
								$form_input = form_textarea(array('rows' => 2, 'name' => $field,'id'=> $field, 'value' => $val ));
							} else {
			...
			echo '<tr><td>&nbsp;</td><td>';
				 echo form_submit(array('value' => 'Edit Phone') );
